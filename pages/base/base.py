...
class Base(object):
    ...
    # by:linguowei
    def find_element_by_android_uiautomator(self, locat: str, elet: str) -> WebElement:
        try:
            wait = WebDriverWait(self.driver, self.timeout)
            element = wait.until(lambda driver: driver.find_element_by_android_uiautomator(f'new UiSelector().{locat}("{elet}")'))
        except (NoSuchElementException, TimeoutException) as e:
            self.logger.error('通过定位表达式{}定位元素{}失败:{}'.format(locat,elet, e))
